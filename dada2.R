...
# Output:
# 175  239  240  241  242  243 
#  4    1  769 5986   55    1 


# Most ASVs are acceptable lengths, but the others must be removed
counts_trimmed <- counts_raw[, nchar(colnames(counts_raw)) %in% seq(239, 243)]

# Filter chimeras
counts_nochim <- removeBimeraDenovo(counts_trimmed, method = "consensus", 
                                    multithread = TRUE, verbose = TRUE)
# Output:
# Identified 4865 bimeras out of 6812 input sequences.
...
# Assign taxonomy
tax_nochim <- assignTaxonomy(counts_nochim, "silva_nr99_v138.1_train_set.fa.gz", multithread = TRUE)

tax_nochim <- addSpecies(tax_nochim, "silva_species_assignment_v138.1.fa.gz")
